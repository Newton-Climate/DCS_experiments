using SpectralFits, vSmartMOM, Dates, DiffResults, ForwardDiff, JLD2, Revise, StatsBase, Random

include("util.jl")
on_fluo = true; # on Fluo server?
...
# save custom p and T
measurement.pressure = 500.0*ones(n)
...
inversion_setup["Sₐ⁻¹"] = make_prior_error(σ, nGases, p.(z), pcorr)
...
ch4_isobar = 1e9*result.x["CH4"] ./ vcd
println(ch4_isobar)
vcd_isobar = vcd

@save "CH4_isobar.jld2" result p0 T0 xₐ vcd_isobar x_true ch4_isobar n
