...
#hit16 = load_data(datadir+'heatmaps/ch4_hit16_results.jld2')
#hit20 = load_data(datadir+'heatmaps/ch4_hit20_results.jld2')
#TCCON = load_data(datadir+'heatmaps/ch4_TCCON_results.jld2')
retrieved = load_data(datadir+'/ch4_broadening_results.jld2')

#var = ['dch4', 'dch4', 'dch4', 'dp', 'dp', 'dp','dT', 'dT', 'dT']
var = ['dch4', 'dp', 'dT']
vmax = [250, 150, 5]#, 15, 15, 15, 0.5, 0.5, 0.5]
vmin = [-250, -150, -5]#, -75, -75, -75, -1, -1, -1]
#array_to_plot = [hit16, hit20, TCCON] * 3
array_to_plot = [retrieved] * 3
cmap = 'PiYG'
...
fig, ax = plt.subplots(nrows=3, ncols=1, sharex = True, sharey = True, figsize = (16,9), dpi = 300)
ax = ax.flatten() 
...
for i in range(len(ax)):
    ...
    if i == 0:
        ...
        ax[i].text(-15, 17, '$CH_4$ Error\n(ppb)', va='center', ha ='center', fontsize = 14)
    if i == 1:
        divnorm = colors.TwoSlopeNorm(vmin=vmin[i], vcenter=0, vmax=vmax[i])
        im2 = ax[1].pcolormesh(array_to_plot[1][var[1]], rasterized=True, norm=divnorm, cmap=cmap,)
        ax[i].set_ylabel('Temperature (K)')
        ax[i].text(-15, 17, 'Pressure Error\n(HPa)', va='center', ha="center", fontsize = 14)

    if i == 2:
        divnorm = colors.TwoSlopeNorm(vmin=vmin[i], vcenter=0, vmax=vmax[i])
        im3 = ax[2].pcolormesh(array_to_plot[2][var[2]], rasterized=True, norm=divnorm, cmap=cmap,)
        ax[i].set_ylabel('Temperature (K)')
        ax[i].text(-15, 17, 'Temperature Error\n(K)', va='center', ha="center", fontsize = 14)
    divider = make_axes_locatable(ax[i])
    cax = divider.append_axes("right", size = "7%", pad = 0.05)
    cax.remove()
    if i == 0:
            cax1 = divider.append_axes("right", size="7%", pad=.05)
            fig.colorbar(pcm, cax=cax1, orientation='vertical', label='Retrieval Error (ppb)')
...
#    ax[2].set_title('TCCON', fontsize = 14)
    ...
    if i > 1: ax[i].set_xlabel('Pressure (HPa)')
    ax[i].grid(False)  
...
plt.tight_layout()
fig.savefig('ch4_params_pert.pdf')
