...
print('Running heatmaps_PRESENTATION.py...')
...
plt.tight_layout()
fig.savefig('figs/presentation/heatmap_ch4_pres.pdf')
print('\tSAVED: figs/presentation/heatmap_ch4_pres.pdf')
plt.close()
...
def calc_errors(data_dict, p_true, T_true):
    ...
    data_dict['dp'] = dp
    data_dict['dT'] = dT
    data_dict['d_co2'] = data_dict['co2'] - 400
    return data_dict
# end of function 
...
vmin = [-15, -15, -15, -15, -15, -15, -1, -1, -1]
array_to_plot = [hit16, hit20, TCCON] * 3
cmap = 'PiYG'

ticks_legend_co2 = [-15, -10, -5, 0, 5, 10, 15]
ticks_legend_p = [-15, -10, -5, 0, 5, 10, 15]
ticks_legend_t = [-1, -0.5, 0, 0.5, 1]
...
for i in range(9):
    ...
    if i == 0:
        ...
        im1 = ax[0].pcolormesh(array_to_plot[0][var[0]], rasterized=True, norm=divnorm, cmap=cmap,)
        ax[i].set_ylabel('Temperature (K)')
        ax[i].text(-15, 17, '$CO_2$\n(ppm)', va='center', ha = "center", fontsize = 14)
    if i == 3:
        divnorm = colors.TwoSlopeNorm(vmin=vmin[i], vcenter=0, vmax=vmax[i])
        im2 = ax[3].pcolormesh(array_to_plot[3][var[3]], rasterized=True, norm=divnorm, cmap=cmap,)
        ax[i].set_ylabel('Temperature (K)')
    ...
    if i == 2:
            cax1 = divider.append_axes("right", size="7%", pad=.05)
            fig.colorbar(pcm, cax=cax1, orientation='vertical', label='Retrieval Error (ppm)', ticks = ticks_legend_co2)
  
    if i == 5:
            cax2 = divider.append_axes("right", size="7%", pad=.05)
            fig.colorbar(pcm, cax=cax2, orientation='vertical',  label='Retrieval Error (HPa)', ticks = ticks_legend_p)
    if i == 8:
            cax3 = divider.append_axes("right", size="7%", pad=.05)
            fig.colorbar(pcm, cax=cax3, orientation='vertical',  label='Retrieval Error (K)', ticks = ticks_legend_t)
    ax[0].set_title('Hitran-2016', fontsize = 14)
    ax[1].set_title('Hitran-2020', fontsize = 14)
    ax[2].set_title('TCCON', fontsize = 14)
    if i > 5: ax[i].set_xlabel('Pressure (HPa)')
    ...
    ax[i].set_xticks(ticks = p_ticks, labels = p_tick_labels)
    ax[i].set_yticks(ticks = t_ticks, labels = t_tick_labels)
    ax[i].set_xlim(1, len(p))
    

plt.subplots_adjust(wspace=0.005, hspace=0)
plt.tight_layout()
fig.savefig('figs/presentation/heatmap_co2_pres.pdf')
print('\tSAVED: figs/presentation/heatmap_co2_pres.pdf')
plt.close()
